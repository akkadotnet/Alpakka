...
namespace Akka.Streams.Kinesis
{
    internal sealed class KinesisSourceStage : GraphStage<SourceShape<Record>>
    {
        ...
        sealed class Logic : TimerGraphStageLogic
        {
            ...
            {
                _self = GetStageActorRef(AwaitingShardIterator);
                GetShardIterator();
            }
            ...
            private void AwaitingShardIterator(Tuple<IActorRef, object> args)
            {
                switch (args.Item2)
                {
                    case GetShardIteratorSuccess s:
                        _iterator = s.Response.ShardIterator;
                        _self.Become(AwaitingRecords);
            ...
            private void AwaitingRecords(Tuple<IActorRef, object> args)
            {
                switch (args.Item2)
                {
            ...
            private void Ready(Tuple<IActorRef, object> args)
            {
                if (args.Item2 is Pump)
                {
                    if (IsAvailable(_out))
                    {
