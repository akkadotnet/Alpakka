...
namespace Akka.Streams.Channels.Internal
{
    ...
    internal sealed class ChannelSinkLogic<T> : InGraphStageLogic
    {
        ...
        public ChannelSinkLogic(SinkShape<T> shape, Inlet<T> inlet, ChannelWriter<T> writer, bool isOwner) : base(shape)
        {
            _inlet = inlet;
            _writer = writer;
            _isOwner = isOwner;
        ...
        public override void OnPush()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (task.IsFaulted)
                    {
                        TryFail(task.Exception);
                    }
                    else if (task.IsCanceled)
                    {
                        TryFail(new TaskCanceledException(task));
                    }
                    else
                    {
                        ...
                        task.ContinueWith(t =>
                        {
                            ...
                            else if (t.IsCanceled) _onWriteFailed(new TaskCanceledException(t));
